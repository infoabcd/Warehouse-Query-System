...
function Category() {
    ...
    const fetchData = async (key) => {
        try {
            ...
            if (key == '0') {
                ...
                const response = await fetch(`http://localhost:3000/search/assort/${key}`);
                if (!response.ok) {
                    throw new Error('网络请求失败');
                }
                const data = await response.json();
    ...
    // 使用 useEffect 监听 `key` 的变化
    useEffect(() => {
        setLoading(true);
        const getData = async () => {
            const fetchedProducts = await fetchData(key);
            setProducts(fetchedProducts);
            setLoading(false);
        };
        getData();
    }, [key]); // 将 `key` 作为依赖项，当它改变时 useEffect 重新运行
